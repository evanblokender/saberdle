...
async function loadDaily() {
  ...
  if(localStorage.getItem(dailyKey)) {
    lockGame("You already played today's Beatdle! Come back tomorrow.");
    return;
  }
...
function playPreview() {
  ...
  if(playPromise !== undefined) {
    playPromise.catch(()=> alert("Click Play Preview to start playback"));
  }
  isPlaying = true;
...
function winGame(){
  ...
  localStorage.setItem(dailyKey,"done"); // lock today
  lockGame("You already played today's Beatdle! Come back tomorrow.");
}
...
function endGame(){
  ...
  shareBtn.hidden=false;
  localStorage.setItem(dailyKey,"done"); // lock today
  lockGame("You already played today's Saberdle! Come back tomorrow.");
}
